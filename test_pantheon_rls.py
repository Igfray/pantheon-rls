...
def test_reserved_word_identifiers_are_quoted():
    # a table named `order` or a role `user` is now valid SQL, not a syntax error (validation still blocks injection)
    assert enable_rls_sql("order")[0] == 'ALTER TABLE "order" ENABLE ROW LEVEL SECURITY;'
    assert grant_crud_sql("order", "user") == 'GRANT SELECT, INSERT, UPDATE, DELETE ON "order" TO "user";'
